...
- **Spring Boot**: モダンなJavaベースのWebアプリケーションの迅速な開発をサポートするフレームワーク。簡潔な設定、組み込みサーバーのサポート、多数のスターターパッケージを提供します。

- **MySQL**: 高性能、高信頼性、および使いやすさを特徴とするオープンソースのリレーショナルデータベース管理システム。

- **MyBatis**: SQL文とオブジェクト間のマッピングを容易にするためのデータアクセスフレームワーク。開発者が直接SQLを記述し、データベース操作を柔軟に制御できます。

- **Mockito**: Javaのモッキングフレームワークで、ユニットテストを書く際に外部の依存関係を模倣するのに役立ちます。テスト対象のオブジェクトが依存するオブジェクトをモック化し、テストの際に予測可能な動作を提供します。
